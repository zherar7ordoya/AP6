using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;
using Entidad;
using BLL;
using Microsoft.Reporting.WinForms;
using System.Windows.Forms;

namespace Controlador_Vista
{
    public class VistaReporteMarcas
    {
        System.Windows.Forms.Form frmReporteMarcas;
        BLL_Marca oBLLMarca;
        List<Control> ListaControles;

        public VistaReporteMarcas(Form pForm)
        {
            try
            {
                ListaControles = new List<Control>();
                // Se guarda en una variable privada la referencia al formulario que llega al parámetro del constructor
                frmReporteMarcas = pForm;

                foreach (System.Windows.Forms.Control c in frmReporteMarcas.Controls)
                {
                    ListaControles.Add(new Control(c.Name, c));
                }

                ((Button)ListaControles.Find(x => x.Nombre == "btn_Volver").Puntero).Click += btn_Volver_Click;
                frmReporteMarcas.Load += frmReporteMarcas_Load;

                oBLLMarca = new BLL_Marca();
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
            }
        }

        #region "Funciones delegadas de eventos UI"

        private void frmReporteMarcas_Load(object sender, EventArgs e)
        {
            ReportViewer reporteMarca = (ReportViewer)ListaControles.Find(x => x.Nombre == "reportViewer1").Puntero;
            try
            {
                reporteMarca.LocalReport.DataSources[0].Value = oBLLMarca.Listar();
                reporteMarca.RefreshReport();
            }
            catch (Exception ex) { throw new Exception(ex.Message); }
        }

        private void btn_Volver_Click(object sender, EventArgs e)
        {
            try
            {
                frmReporteMarcas.Close();
            }
            catch (Exception ex) { throw new Exception(ex.Message); }
        }
        #endregion
    }
}
